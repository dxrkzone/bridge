...
const TOKENS = {
  ...
  USDT: { symbol: 'USDT', name: 'Tether USD', decimals: 6, addresses: {
  ...
  DAI: { symbol: 'DAI', name: 'Dai Stablecoin', decimals: 18, addresses: {
...
// Security Manager - Protects against common attack vectors
class SecurityManager {
  constructor() {
    ...
    this.RATE_LIMIT_MS = 5000; // 5 seconds between transactions
  }
  ...
  // Rate limiting to prevent rapid successive transactions
  checkRateLimit() {
    const now = Date.now();
  ...
  // Detect transaction hash collision
  ...
  // Check if contract interaction is suspicious
  async validateContract(address, rpc) {
    try {
      ...
      if (data.result === '0x' || data.result === '0x0') {
        return false; // Not a contract, might be suspicious
      }
      return true;
    } catch (e) {
      return false;
